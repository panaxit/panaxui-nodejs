...
Class.prototype.getXML = function(callback) {
	...
	sql.connect(config.db, function (err) {
		...
		var sql_req = new sql.Request();
		var sql_str = 'EXEC [$Ver:' + config.db.version + '].getXmlData ' + that.toString();
		...
		sql_req.query(sql_str, function (err, recordset) {
			...
			if(!xml)
				return callback({message: "PanaxJS Error: Missing XML Data"});
...
Class.prototype.getFilename = function(catalog, callback) {
	var sLocation = path.join(
		config.ui.guis[this.options.output].cache,
		catalog.dbId,
		catalog.lang,
